...
export default function useWidgetMenu({ config = defaultConfig }: Partial<useWidgetMenuConfig> = {}) {
  ...
  usePropertyMenu(
    [
      { itemType: "dropdown", propertyName: "chat", tooltip: "Change Context (Recipient)", selectedOption: chatPresets[chatId].option, options: [...chatPresets] },
      { itemType: "dropdown", propertyName: "display", tooltip: "Chat Display Mode", selectedOption: displayOptions[displayMode].option, options: [...displayOptions] },
      ...optionalMenuItem(
        { itemType: "dropdown", propertyName: "viewport", tooltip: "Viewport Dimensions chat", selectedOption: viewportDimensions[viewport].option, options: [...viewportDimensions] },
        displayMode === 1,
      ),
      { itemType: "separator" },
    ...
    ({ propertyName, propertyValue }) => {
      try {
        switch (propertyName) {
          case "chat":
            setChatId(chatPresets.findIndex((o) => o.option === propertyValue))
            break
          case "display":
            setDisplayMode(displayOptions.findIndex((o) => o.option === propertyValue))
            break
          case "viewport":
            setViewport(viewportDimensions.findIndex((o) => o.option === propertyValue))
            break
          case "theme":
            setTheme((prev) => THEME_MODES[(THEME_MODES.findIndex((m) => m === prev) + 1) % THEME_MODES.length])
            break
          case "edit":
          ...
          case "uploadAvatar": {
            ...
            if (hash || data) {
              ...
              figma.notify?.(`Avatar updated from vault${hash ? " (hash)" : ""}${data ? " (dataURL)" : ""}`)
            } else {
              figma.notify?.('No avatar in vault. Run "Telegram Widget — Asset Vault" → Upload Avatar')
            }
            break
          }
          ...
          case "uploadWallpaper": {
            ...
            if (hash || data) {
              ...
              figma.notify?.(`Wallpaper updated from vault${hash ? " (hash)" : ""}${data ? " (dataURL)" : ""}`)
            } else {
              figma.notify?.('No wallpaper in vault. Run "Telegram Widget — Asset Vault" → Upload Wallpaper')
            }
            break
          }
        ...
        figma.notify?.("Menu handler error: " + (err as Error).message)
      }
    },
  )
