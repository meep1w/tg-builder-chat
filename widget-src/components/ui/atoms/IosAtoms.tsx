// widget-src/components/ui/atoms/iosAtoms.tsx

// Dependencies
const { Frame, AutoLayout, Rectangle, SVG, Text, useSyncedState } = figma.widget
// Components
import { remapTokens } from "@/utils"

/** Import Changelog
 * - Status time editable via right panel: useSyncedState("statusTime")
 * - Dynamic battery icon: percent, white fill (red <=20%), 0% empty
 * - SIZE UPDATE: battery 23x11 (was 28x13)
 */
...
export function IosHeaderStatus({ theme, ...props }: IosHeaderProps) {
  ...
  // Время статус-бара — редактируемое из правой панели
  const [statusTime] = useSyncedState<string>("statusTime", "9:41")

  // Процент батареи — редактируется в MessageBuilder
  const [batteryPercent] = useSyncedState<number>("batteryPercent", 100)

  const clamp = (n: number, min: number, max: number) => Math.max(min, Math.min(max, n))
  const pct = clamp(Math.round(batteryPercent ?? 0), 0, 100)

  // Цвета батареи
  const iosRed = "#FF453A"
  const isLow = pct > 0 && pct <= 20
  const hasFill = pct > 0
  const fillColor = isLow ? iosRed : "#FFFFFF"
  const textColor = pct === 0 ? iosRed : (isLow ? "#FFFFFF" : "#000000")

  // SVG-иконки статуса (без изменений)
  const svgPaths = {
  ...
  // SVG для батарейки (viewBox 0 0 23 11)
  // Серый контур (нижний слой)
  const grayBatterySVG = `
<svg width="23" height="11" viewBox="0 0 23 11" fill="none" xmlns="http://www.w3.org/2000/svg">
  <rect width="21.0407" height="11" rx="4.3" fill="#767676"/>
  <path d="M21.8823 4.23096V7.67943C22.5595 7.38737 23 6.70794 23 5.9552C23 5.20245 22.5595 4.52302 21.8823 4.23096" fill="#767676"/>
</svg>`

  // Белый/красный верхний слой — цвет подставляем динамически
  const coloredBatterySVG = (fill: string) => `
<svg width="23" height="11" viewBox="0 0 23 11" fill="none" xmlns="http://www.w3.org/2000/svg">
  <rect width="21.1538" height="11" rx="4.3" fill="${fill}"/>
  <path d="M21.8823 4.23083V7.67931C22.5595 7.38725 23 6.70782 23 5.95507C23 5.20233 22.5595 4.5229 21.8823 4.23083" fill="${fill}"/>
</svg>`

  // Геометрия клипа для 23x11
  const BATTERY_W = 23
  const BATTERY_H = 11
  const clipWidth = Math.round((pct / 100) * BATTERY_W)
  ...
  return (
    ...
    >
      ...
      <AutoLayout name="Indicators" overflow="visible" spacing={8} verticalAlignItems="center">
        <SVG name="Mobile Signal" height={11} width={17} src={svgPaths.mobile} />
        <SVG name="Wifi" height={11} width={15} src={svgPaths.wifi} />

        {/* Battery 23x11 */}
        <Frame name="Battery" width={BATTERY_W} height={BATTERY_H} overflow="visible" strokeWidth={0}>
          {/* Нижний серый слой */}
          <SVG name="Battery/Gray" width={BATTERY_W} height={BATTERY_H} src={grayBatterySVG} />

          {/* Верхний слой с обрезкой по ширине = процент */}
          {hasFill && (
            <Frame name="Battery/FillClip" width={clipWidth} height={BATTERY_H} overflow="hidden" strokeWidth={0} x={0} y={0}>
              <SVG name="Battery/Fill" width={BATTERY_W} height={BATTERY_H} src={coloredBatterySVG(fillColor)} />
            </Frame>
          )}

          {/* Процент поверх — чутка меньше шрифт */}
          <AutoLayout
            name="Battery/Label"
            x={0}
            y={0}
...
export function IosBottomBar({ theme, ...props }: IosBottomBarProps) {
  // Theme Mode
  const color = remapTokens({
    surface: { bar: { dark: "#FFF", light: "#000" } },
    text: {},
  })[theme]
  ...
  return (
    <Frame name="IosBottomBar" width={375} height={34} {...props}>
      <Rectangle
        name="Background"
        x={{ type: "horizontal-scale", leftOffsetPercent: 0, rightOffsetPercent: 0 }}
        y={{ type: "vertical-scale", topOffsetPercent: 0, bottomOffsetPercent: 0 }}
        strokeWidth={0}
        width={375}
        height={34}
      />
